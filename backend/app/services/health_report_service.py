...
class HealthReportService:
    ...
    def create(db: Session, report_in: HealthReportCreate) -> HealthReportResponse:
        ...
        rep = HealthReport(
            ...
            duration_days=report_in.duration_days,
            number_of_animals_affected=report_in.number_of_animals_affected,
            latitude=report_in.latitude or (user.latitude if user else 18.1515),
            longitude=report_in.longitude or (user.longitude if user else 74.5772),
            village=report_in.village or (user.village if user else "Baramati"),
            district=report_in.district or (user.district if user else "Pune"),
            risk_score=score,
            risk_level=level,
