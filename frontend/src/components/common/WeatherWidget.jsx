  ...
  return (
    <div className={`rounded-2xl border p-4.5 transition-all ${
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-800/80">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">{globalT('environment.sensorMesh')}</p>
          </div>
        </div>
        <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-full border ${
          ...
          {globalT('data.risk.' + (weather?.seasonal_risk || 'Moderate'), {}, weather?.seasonal_risk || 'Moderate')} Seasonal Risk
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2.5 my-3 text-center">
        <div className="bg-slate-950/60 border border-slate-800/60 rounded-xl p-2">
          <div className="flex items-center justify-center space-x-1 text-slate-400 text-[10px]">
            ...
            <span>{globalT('environment.temp')}</span>
          </div>
        ...
        <div className="bg-slate-950/60 border border-slate-800/60 rounded-xl p-2">
          <div className="flex items-center justify-center space-x-1 text-slate-400 text-[10px]">
            ...
            <span>{globalT('environment.humidity')}</span>
          </div>
        ...
        <div className="bg-slate-950/60 border border-slate-800/60 rounded-xl p-2">
          <div className="flex items-center justify-center space-x-1 text-slate-400 text-[10px]">
            ...
            <span>{globalT('environment.rainfall')}</span>
          </div>
          <span className="text-sm font-black text-slate-300 mt-0.5 block">
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-2.5 text-[11px] text-amber-200/90 leading-relaxed flex items-start space-x-2">
        ...
        <span>{globalT('environment.riskDescription')}</span>
      </div>
    </div>
  )
}
